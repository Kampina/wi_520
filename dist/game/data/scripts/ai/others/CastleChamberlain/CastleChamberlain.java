...
public class CastleChamberlain extends Script
{
	...
	private static final Map<Integer, List<Integer>> CLAN_HALLS = new HashMap<>();
	static
	{
		...
		CLAN_HALLS.put(1, Arrays.asList(22, 23, 24, 25, 26, 27, 28, 29, 30, 65, 66, 67, 68)); // Gludio
		CLAN_HALLS.put(2, Arrays.asList(31, 32, 33, 69, 70, 71, 72, 73, 74)); // Dion
		CLAN_HALLS.put(3, Arrays.asList(42, 43, 44, 45, 46)); // Giran
		CLAN_HALLS.put(5, Arrays.asList(36, 37, 38, 39, 40, 41)); // Aden
		CLAN_HALLS.put(7, Arrays.asList(47, 48, 49, 50)); // Goddard
	}
	...
	private int getProductBuyListId(Npc npc)
	{
		return Integer.parseInt(npc.getId() + "1");
	}

	private boolean hasProductBuyList(Npc npc)
	{
		return BuyListData.getInstance().getBuyList(getProductBuyListId(npc)) != null;
	}
	...
	private List<Integer> getManagedClanHallIds(Castle castle)
	{
		return CLAN_HALLS.getOrDefault(castle.getResidenceId(), List.of());
	}
	...
	public String onEvent(String event, Npc npc, Player player)
	{
		...
		final boolean isMyLord = (clan != null) && player.isClanLeader() && (clan.getCastleId() == (npc.getCastle() != null ? npc.getCastle().getResidenceId() : -1));
		...
		switch (st.nextToken())
		{
			...
			case "view_territory_war_info":
			{
				if (isMyLord)
				{
					player.sendPacket(new ExShowDominionRegistry(player, castle.getResidenceId() + TERRITORY_ID_OFFSET));
				}
				else
				{
			...
			case "manager_contribution":
			{
				if (isMyLord)
				{
					final NpcHtmlMessage html = getHtmlPacket(player, npc, "chamberlain-contribution.html");
					html.replace("%epaulettes%", String.valueOf(castle.getFortressContributionEpaulettes()));
					player.sendPacket(html);
				}
				else
				{
			...
			case "receive_contribution_epaulettes":
			{
				if (isMyLord)
				{
					final long epaulettes = castle.takeFortressContributionEpaulettes();
					if (epaulettes > 0)
					{
						player.addItem(ItemProcessType.REWARD, 9912, epaulettes, npc, true);
					}
					onEvent("manager_contribution", npc, player);
				}
				else
				{
					htmltext = "chamberlain-21.html";
			...
			case "receive_report":
			{
				if (isMyLord)
				{
					...
					else
					{
						...
						html.replace("%clanleadername%", ownerClan.getLeaderName());
						html.replace("%clanname%", ownerClan.getName());
			...
			case "manage_clan_halls":
			{
				...
				final NpcHtmlMessage html = getHtmlPacket(player, npc, "chamberlain-clanhall.html");
				final StringBuilder buttons = new StringBuilder();
				for (int clanHallId : getManagedClanHallIds(castle))
				{
					...
					if (ClanHallAuctionManager.getInstance().getClanHallAuctionById(clanHallId) != null)
					{
						buttons.append("<a action=\"bypass -h Quest CastleChamberlain manage_clan_halls_selected ").append(clanHallId).append("\">").append(clanHall.getName()).append("</a><br>");
					}
				}

				if (buttons.length() == 0)
				{
					buttons.append("There are no active clan hall auctions under this castle.");
				}

				html.replace("%button_list%", buttons.toString());
				player.sendPacket(html);
				break;
			}
			case "manage_clan_halls_selected":
			{
				if (!isMyLord)
				{
					htmltext = "chamberlain-21.html";
				...
				if (!st.hasMoreTokens())
				{
					onEvent("manage_clan_halls", npc, player);
					break;
				}

				final int clanHallId = Integer.parseInt(st.nextToken());
				final ClanHall clanHall = ClanHallData.getInstance().getClanHallById(clanHallId);
				final ClanHallAuction auction = ClanHallAuctionManager.getInstance().getClanHallAuctionById(clanHallId);

				final NpcHtmlMessage html = getHtmlPacket(player, npc, "chamberlain-clanhall-selected.html");
				html.replace("%clan_hall_name%", (clanHall != null) ? clanHall.getName() : "Unknown");
				final StringBuilder buttons = new StringBuilder();
				if ((clanHall != null) && getManagedClanHallIds(castle).contains(clanHallId) && (auction != null) && (auction.getRemainingTime() > 0))
				{
					for (Bidder bidder : auction.getSortedBidders())
					{
						buttons.append("<Button ALIGN=LEFT ICON=\"NORMAL\" action=\"bypass -h Quest CastleChamberlain manage_clan_halls_clan_selected ").append(clanHallId).append(" ").append(bidder.getClan().getId()).append("\">")
							.append(bidder.getClanName())
							.append(" (Bid: ")
							.append(FormatUtil.formatAdena(bidder.getBid()))
							.append(")</Button>");
					}
				}
				...
				if (buttons.length() == 0)
				{
					buttons.append("There are no registered clans for this clan hall auction.");
				}
			...
			case "manage_clan_halls_clan_selected":
			{
				if (!isMyLord)
				{
					htmltext = "chamberlain-21.html";
				...
				if (!st.hasMoreTokens())
				{
					onEvent("manage_clan_halls_selected " + clanHallId, npc, player);
					break;
				}

				final int clanId = Integer.parseInt(st.nextToken());
				if (!getManagedClanHallIds(castle).contains(clanHallId))
				{
					onEvent("manage_clan_halls", npc, player);
					break;
				}
				...
				if (!ClanHallAuctionManager.getInstance().selectAuctionWinner(clanHallId, clanId))
				{
					onEvent("manage_clan_halls_selected " + clanHallId, npc, player);
					break;
				}
			...
			case "change_tax":
			{
				player.sendMessage("Налоги с городов отключены.");
				break;
			}
			case "manor":
			{
			...
			case "products":
			{
				if (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_FUNCTIONS))
				{
					...
					html.replace("%buyItemButton%", hasProductBuyList(npc) ? "<Button ALIGN=LEFT ICON=\"NORMAL\" action=\"bypass -h Quest CastleChamberlain buy %npcId%1\">Buy an item</Button>" : "");
			...
			case "buy":
			{
				if (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_FUNCTIONS))
				{
					final int buyListId = Integer.parseInt(st.nextToken());
					if (BuyListData.getInstance().getBuyList(buyListId) != null)
					{
						((Merchant) npc).showBuyWindow(player, buyListId);
					}
					else
					{
						final NpcHtmlMessage html = getHtmlPacket(player, npc, "chamberlain-22.html");
						html.replace("%buyItemButton%", "");
						html.replace("%npcId%", Integer.toString(npc.getId()));
						player.sendPacket(html);
					}
				}
