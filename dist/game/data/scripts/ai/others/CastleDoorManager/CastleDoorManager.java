...
public class CastleDoorManager extends Script
{
	...
	public String onEvent(String event, Npc npc, Player player)
	{
		...
		switch (action)
		{
			case "manageDoors":
			{
				if (canUseDoorManager(player, npc) && st.hasMoreTokens())
				{
	...
	{
		return canUseDoorManager(player, npc) ? getHtmlName(npc) + ".html" : getHtmlName(npc) + "-no.html";
	}
	...
	private boolean canUseDoorManager(Player player, Npc npc)
	{
		return isOwningClan(player, npc) && player.hasAccess(ClanAccess.CASTLE_OPEN_DOOR);
	}
