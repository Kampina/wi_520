...
public class CastleTeleporter extends Script
{
	...
	public String onEvent(String event, Npc npc, Player player)
	{
		...
		switch (action)
		{
			...
			case "teleportMe":
			{
				if (ArrayUtil.contains(SIEGE_TELEPORTERS, npc.getId()) && !canUseSiegeTeleporter(player, npc))
				{
					return getHtmlName(npc) + "-no.html";
				}
				if (st.hasMoreTokens())
				{
	...
	public String onFirstTalk(Npc npc, Player player)
	{
		...
		else if (canUseSiegeTeleporter(player, npc)) // Defender
		{
	...
	private boolean canUseSiegeTeleporter(Player player, Npc npc)
	{
		return player.isGM() || ((npc.getCastle().getOwnerId() == player.getClanId()) && (player.getClanId() != 0) && (player.getSiegeState() == 2));
	}
