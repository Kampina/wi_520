...
public class BotReport implements IPlayerActionHandler
{
	...
	public void onAction(Player player, ActionDataHolder data, boolean ctrlPressed, boolean shiftPressed)
	{
		...
		{
			final int minLevel = GeneralConfig.BOTREPORT_REPORTER_LEVELS[0];
			final int maxLevel = GeneralConfig.BOTREPORT_REPORTER_LEVELS[1];
			if ((player.getLevel() < minLevel) || (player.getLevel() > maxLevel))
			{
				player.sendMessage("You do not meet the level requirement to report bots.");
				return;
			}
