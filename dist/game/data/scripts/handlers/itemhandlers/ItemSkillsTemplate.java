...
public class ItemSkillsTemplate implements IItemHandler
{
	...
	public boolean onItemUse(Playable playable, Item item, boolean forceUse)
	{
		...
		// Potions can only be used while sitting.
		if (playable.isPlayer() && item.getTemplate().isPotion() && !playable.asPlayer().isSitting())
		{
			playable.asPlayer().sendMessage("Зелья можно использовать только сидя.");
			return false;
		}
		...
		for (SkillHolder skillInfo : skills)
		{
			...
			if (itemSkill != null)
			{
				...
				if (playable.isPet())
				{
					final SystemMessage sm = new SystemMessage(SystemMessageId.YOUR_GUARDIAN_IS_USING_S1);
