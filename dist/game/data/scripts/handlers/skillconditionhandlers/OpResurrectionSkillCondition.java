...
public class OpResurrectionSkillCondition implements ISkillCondition
{
	...
	public boolean canUse(Creature caster, Skill skill, WorldObject target)
	{
		...
		if (target.isPlayer())
		{
			...
			if ((skill.getId() == 1254) && caster.isPlayer() && (caster.isInsideZone(ZoneId.SIEGE) || player.isInsideZone(ZoneId.SIEGE)))
			{
				final Player casterPlayer = caster.asPlayer();
				if ((casterPlayer.getParty() == null) || (player.getParty() == null) || (casterPlayer.getParty() != player.getParty()))
				{
					canResurrect = false;
					caster.sendPacket(SystemMessageId.S1_CANNOT_BE_USED_THE_REQUIREMENTS_ARE_NOT_MET);
				}
			}
