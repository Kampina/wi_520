...
public class ClanBoard implements IWriteBoardHandler
{
	...
	public boolean onCommand(String command, Player player)
	{
		...
		else if (command.startsWith("_bbsclan_clanhome"))
		{
			...
			else if (command.startsWith("_bbsclan_clanhome;"))
			{
				try
				{
					final String[] split = command.split(";", 3);
					final int clanId = Integer.parseInt(split[1]);
					final String route = split.length > 2 ? split[2].toLowerCase() : "";
					switch (route)
					{
						case "announce":
						case "cnotice":
						{
							clanNotice(player, clanId);
							break;
						}
						case "cbb":
						{
							showCompatibilityPage(player, clanId, "Clan Bulletin Board", "The legacy clan bulletin board owner is not present in this RV branch.");
							break;
						}
						case "cmail":
						{
							showCompatibilityPage(player, clanId, "Clan Mail", "The legacy clan mail runtime is not present in this RV branch.");
							break;
						}
						default:
						{
							clanHome(player, clanId);
							break;
						}
					}
				}
		...
		else if (command.startsWith("_clbbsclan_"))
		{
			CommunityBoardHandler.getInstance().addBypass(player, "Legacy Clan Home", command);
			try
			{
				clanHome(player, Integer.parseInt(command.substring("_clbbsclan_".length())));
			}
			catch (Exception e)
			{
				clanList(player, 1);
				LOG.warning(ClanBoard.class.getSimpleName() + ": " + player + " send invalid legacy clan home bypass " + command + "!");
			}
		}
		else if (command.startsWith("_clbbslist_"))
		{
			CommunityBoardHandler.getInstance().addBypass(player, "Legacy Clan List", command);
			try
			{
				final String[] split = command.substring("_clbbslist_".length()).split("_");
				final int page = Integer.parseInt(split[0]);
				clanList(player, page);
			}
			catch (Exception e)
			{
				clanList(player, 1);
				LOG.warning(ClanBoard.class.getSimpleName() + ": " + player + " send invalid legacy clan list bypass " + command + "!");
			}
		}
		else if (command.equals("_announcepledgewriteform"))
		{
			CommunityBoardHandler.getInstance().addBypass(player, "Legacy Clan Notice", command);
		...
		else if (command.equals("_clbbsadmi"))
		{
			CommunityBoardHandler.getInstance().addBypass(player, "Legacy Clan Admin", command);
			showCompatibilityPage(player, player.getClanId(), "Clan Management", "The legacy clan introduction editor owner is not present in this RV branch.");
		}
		else if (command.equals("_mailwritepledgeform"))
		{
			CommunityBoardHandler.getInstance().addBypass(player, "Legacy Clan Mail", command);
			showCompatibilityPage(player, player.getClanId(), "Clan Mail", "The legacy clan mail compose owner is not present in this RV branch.");
		}
	...
	private void clanHome(Player player, int clanId)
	{
		...
		if (cl != null)
		{
			...
			else
			{
				 ...
				 */
					"<tr><td height=7></td></tr><tr><td fixWIDTH=100 align=left>ALLIANCE</td><td fixWIDTH=195 align=left height=16>", cl.getDisplayAllyName(), "</td></tr></table></td><td fixWIDTH=5></td></tr><tr><td height=10></td></tr></table>" +
	...
	private void showCompatibilityPage(Player player, int clanId, String featureName, String message)
	{
		final int resolvedClanId = clanId > 0 ? clanId : ((player.getClan() != null) ? player.getClanId() : 0);
		final StringBuilder html = new StringBuilder(1024);
		html.append("<html><body><br><br><center><table width=610 border=0 cellspacing=0 cellpadding=0><tr><td fixwidth=610 align=left><a action=\"bypass _bbshome\">HOME</a> &gt; <a action=\"bypass _bbsclan_clanlist\">CLAN COMMUNITY</a>");
		if (resolvedClanId > 0)
		{
			html.append(" &gt; <a action=\"bypass _bbsclan_clanhome;");
			html.append(resolvedClanId);
			html.append("\">&amp;$802;</a>");
		}
		html.append("</td></tr></table><br><img src=\"L2UI.Squaregray\" width=\"610\" height=\"1\"><br><br><table width=610 border=0 cellspacing=0 cellpadding=0><tr><td align=center><font color=\"LEVEL\">");
		html.append(featureName);
		html.append("</font></td></tr><tr><td height=10></td></tr><tr><td align=center>");
		html.append(message);
		html.append("</td></tr><tr><td height=15></td></tr><tr><td align=center>The existing RV clan owner currently supports clan list, clan home, and clan notice only.</td></tr></table><br><button value=\"Back\" action=\"bypass _bbsclan_clanhome;");
		html.append(resolvedClanId);
		html.append("\" back=\"l2ui_ch3.smallbutton2_down\" width=65 height=20 fore=\"l2ui_ch3.smallbutton2\"></center></body></html>");
		CommunityBoardHandler.separateAndSend(html.toString(), player);
	}
