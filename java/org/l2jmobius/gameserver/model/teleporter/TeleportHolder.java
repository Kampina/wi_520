...
public class TeleportHolder
{
	...
	public void showTeleportList(Player player, Npc npc, String bypass)
	{
		...
		{
			// Skip teleports to enemy faction cities.
			if (isFactionBlockedTeleport(player, loc))
			{
				continue;
			}
	...
	public void doTeleport(Player player, Npc npc, int locId)
	{
		...
		if (!org.l2jmobius.gameserver.model.teleporter.CustomTeleportRules.canTeleport(player, npc, loc, isNoblesse()))
		{
			return;
		}
		...
		if (isFactionBlockedTeleport(player, loc))
		{
			player.sendMessage("Вы не можете телепортироваться в город враждебной фракции.");
			return;
		}
		...
		if (isNormalTeleport())
		{
			...
			else if (!PlayerConfig.ALT_GAME_KARMA_PLAYER_CAN_USE_GK && (player.getReputation() < 0))
			{
		...
		if (shouldPayFee(player, loc) && !player.destroyItemByItemId(ItemProcessType.FEE, loc.getFeeId(), calculateFee(player, loc), npc, true))
		{
			...
			{
				player.sendMessage("You do not have enough " + getItemName(loc.getFeeId(), true) + ".");
			}
			return;
		}
		
		org.l2jmobius.gameserver.model.Location finalLoc = org.l2jmobius.gameserver.model.teleporter.CustomTeleportRules.getFinalLocation(player, loc);
	...
	/**
	 * Checks if the teleport destination is blocked for the player's faction.<br>
	 * A destination linked to a castle is blocked if the castle side is enemy faction.
	 * @param player the player
	 * @param loc the teleport location
	 * @return {@code true} if teleport is blocked
	 */
    private boolean isFactionBlockedTeleport(Player player, TeleportLocation loc)
    {
        return false;
    }
}
