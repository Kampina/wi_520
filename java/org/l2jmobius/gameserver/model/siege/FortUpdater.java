		...
		try
		{
			switch (_updaterType)
			{
				case PERIODIC_UPDATE:
				{
					...
					final long generatedEpaulettes = Math.max(0, FeatureConfig.KNIGHTSEPAULETTE_POINTS);
					if (generatedEpaulettes > 0)
					{
						long castleTax = 0;
						long fortShare = generatedEpaulettes;
						if (_fort.getFortState() == 2)
						{
							final Castle contractedCastle = _fort.getContractedCastle();
							if (contractedCastle != null)
							{
								castleTax = generatedEpaulettes / 2;
								fortShare = generatedEpaulettes - castleTax;
								if (castleTax > 0)
								{
									contractedCastle.addFortressContributionEpaulettes(castleTax);
								}
							}
						}

						_fort.addEpaulettes(fortShare);
					}
