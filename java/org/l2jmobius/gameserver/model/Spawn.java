...
public class Spawn extends Location
{
	...
	public Spawn(NpcTemplate template, NpcSpawnTemplate npcSpawnTemplate) throws ClassNotFoundException, NoSuchMethodException, ClassCastException
	{
		super(0, 0, -10000);
		...
		final String className = "org.l2jmobius.gameserver.model.actor.instance." + resolveImplementationName(_template, npcSpawnTemplate);
	...
	private String resolveImplementationName(NpcTemplate template, NpcSpawnTemplate npcSpawnTemplate)
	{
		if ((npcSpawnTemplate != null) && "Folk".equals(template.getType()) && "Doorman".equalsIgnoreCase(template.getName()))
		{
			final File spawnFile = npcSpawnTemplate.getSpawnTemplate().getFile();
			if ((spawnFile != null) && "fort_doormen.xml".equalsIgnoreCase(spawnFile.getName()))
			{
				return "FortDoorman";
			}
		}
	...
	private boolean isFortressSiegeSpawn()
	{
		if ((_spawnTemplate == null) || (_spawnTemplate.getSpawnTemplate() == null) || (_spawnTemplate.getSpawnTemplate().getFile() == null))
		{
			return false;
		}

		final String normalizedPath = _spawnTemplate.getSpawnTemplate().getFile().getPath().replace('\\', '/').toLowerCase();
		return normalizedPath.contains("/spawns/fortress/siege/");
	}

	private void applyFortressNpcOverrides(Npc npc, int x, int y, int z)
	{
		if (!isFortressSiegeSpawn())
		{
			return;
		}

		final Fort fort = FortManager.getInstance().getFort(x, y, z);
		if (fort == null)
		{
			return;
		}

		npc.setClanId(3000 + Math.max(0, fort.getResidenceId() - 100));
	...
	private Npc initializeNpc(Npc npc)
	{
		...
		npc.setSpawn(this);
		applyFortressNpcOverrides(npc, newlocx, newlocy, newlocz);
