...
public class ClanWar
{
	...
	public void onKill(Player killer, Player victim)
	{
		...
		{
			final int reputationPoints = FeatureConfig.getClanWarReputationPoints(killer.getLevel(), victim.getLevel());
			// however, when the other side reputation score is 0 or below, your clan cannot acquire any reputation points from them.
			if ((victimClan.getReputationScore() > 0) && (killerClan.getReputationScore() > 0) && (victimClan.getLevel() >= 5) && (killerClan.getLevel() >= 5))
			{
				victimClan.takeReputationScore(reputationPoints);
				killerClan.addReputationScore(reputationPoints);
			}
