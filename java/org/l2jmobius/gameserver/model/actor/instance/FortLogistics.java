...
public class FortLogistics extends Merchant
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		final boolean isMyLord = (clan != null) && player.isClanLeader() && (clan.getFortId() == (getFort() != null ? getFort().getResidenceId() : -1));
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		if (actualCommand.equalsIgnoreCase("rewards") || actualCommand.equalsIgnoreCase("rewardInfo"))
		{
			if (isMyLord)
			{
				html.setFile(player, "data/html/fortress/logistics-rewards.htm");
				html.replace("%bloodoath%", String.valueOf(clan.getBloodOathCount()));
				html.replace("%epaulettes%", String.valueOf(getFort().getEpaulettes()));
			}
			else
			{
		...
		else if (actualCommand.equalsIgnoreCase("receiveRewards"))
		{
			if (isMyLord)
			{
				...
				final long epaulettes = getFort().getEpaulettes();
				if ((blood > 0) || (epaulettes > 0))
				{
					if ((blood > 0) && (clan.getLevel() < 9))
					{
					...
					if (epaulettes > 0)
					{
						player.addItem(ItemProcessType.QUEST, 9912, epaulettes, this, true);
						getFort().setEpaulettes(0);
		...
		else if (actualCommand.equalsIgnoreCase("blood") || actualCommand.equalsIgnoreCase("obtainepaulettes"))
		{
			if (isMyLord)
			{
				if (actualCommand.equalsIgnoreCase("obtainepaulettes"))
				{
					final long epaulettes = getFort().getEpaulettes();
					if (epaulettes > 0)
					{
