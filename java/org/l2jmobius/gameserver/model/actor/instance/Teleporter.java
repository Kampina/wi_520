...
public class Teleporter extends Merchant
{
	...
	{
		if (org.l2jmobius.gameserver.model.siege.Siege.isAnyDominionBattlefieldActive())
		{
			if (command.startsWith("showTeleportList") || command.startsWith("showTeleportsHunting") || command.startsWith("teleport"))
			{
				player.sendMessage("Услуги Gatekeeper отключены на время проведения Осады.");
				return;
			}
		}
		...
		switch (st.nextToken())
		{
			case "Util:NoblessTeleport": { final TeleportHolder holder = TeleporterData.getInstance().getHolder(getId(), TeleportType.NOBLES_ADENA.name()); if (holder != null) holder.showTeleportList(player, this); break; } case "showNoblesSelect":
			{
				sendHtmlMessage(player, "data/html/teleporter/" + (player.isNoble() ? "nobles_select" : "not_nobles") + ".htm");
				break;
			}
			case "Teleport": case "showTeleports":
			{
				final String listName = (st.hasMoreTokens()) ? st.nextToken() : TeleportType.NORMAL.name();
				final TeleportHolder holder = TeleporterData.getInstance().getHolder(getId(), (listName.equals("1") ? TeleportType.NORMAL.name() : listName));
				if (holder == null)
				{
