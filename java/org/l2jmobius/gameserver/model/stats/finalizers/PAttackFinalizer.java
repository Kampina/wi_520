...
public class PAttackFinalizer implements IStatFunction
{
	...
	public double calc(Creature creature, OptionalDouble base, Stat stat)
	{
		...
		{
			baseValue *= (creature.isAttackable() && (creature.getChampionTier() == 2)) ? ChampionMonstersConfig.CHAMPION_ATK_RED : ChampionMonstersConfig.CHAMPION_ATK;
		}
		...
		// Fire attribute bonus (physical damage %)
		if (creature.isPlayer())
		{
			final int fireAttr = creature.getStat().getEquippedAttributeTotal(AttributeType.FIRE);
			if (fireAttr > 0)
			{
				baseValue *= 1.0 + (fireAttr * 0.01);
			}
		}
