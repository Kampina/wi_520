...
import org.l2jmobius.gameserver.model.stats.finalizers.PCriticalDamageFinalizer;
...
public enum Stat
{
	...
	CRITICAL_DAMAGE("cAtk", new PCriticalDamageFinalizer()),
	...
	FIRE_ABSORB("fireAbsorb"),
	WATER_ABSORB("waterAbsorb"),
	WIND_ABSORB("windAbsorb"),
	EARTH_ABSORB("earthAbsorb"),
	HOLY_ABSORB("holyAbsorb"),
	DARK_ABSORB("darkAbsorb"),
	...
	{
		String internName = normalizeXmlName(name).intern();
		for (Stat s : values())
		{
	...
	private static String normalizeXmlName(String name)
	{
		switch (name)
		{
			case "pvpMagicSkillDefenceBonus":
			{
				return PVP_MAGICAL_SKILL_DEFENCE.getValue();
			}
			case "pvpPhysSkillDefenceBonus":
			{
				return PVP_PHYSICAL_SKILL_DEFENCE.getValue();
			}
			case "pvpPhysDefenceBonus":
			{
				return PVP_PHYSICAL_ATTACK_DEFENCE.getValue();
			}
			case "pvpMagicSkillDmgBonus":
			{
				return PVP_MAGICAL_SKILL_DAMAGE.getValue();
			}
			case "pvpPhysSkillDmgBonus":
			{
				return PVP_PHYSICAL_SKILL_DAMAGE.getValue();
			}
			case "pvpPhysDmgBonus":
			{
				return PVP_PHYSICAL_ATTACK_DAMAGE.getValue();
			}
			case "cAtkStatic":
			{
				return CRITICAL_DAMAGE_ADD.getValue();
			}
			case "healPower":
			{
				return HEAL_EFFECT.getValue();
			}
			case "atkBaseSpeed":
			{
				return PHYSICAL_ATTACK_SPEED.getValue();
			}
			default:
			{
				return name;
