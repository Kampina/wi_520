...
public class ClanHall extends AbstractResidence
{
	...
	Clan _owner = null;
	long _paidUntil = 0;
	long _auctionEndTime = 0;
	protected ScheduledFuture<?> _checkPaymentTask = null;
	
	// Other
	private static final String INSERT_CLANHALL = "INSERT INTO clanhall (id, ownerId, paidUntil, auctionEndTime) VALUES (?,?,?,?)";
	private static final String LOAD_CLANHALL = "SELECT * FROM clanhall WHERE id=?";
	private static final String UPDATE_CLANHALL = "UPDATE clanhall SET ownerId=?,paidUntil=?,auctionEndTime=? WHERE id=?";
	...
	public ClanHall(StatSet params)
	{
		...
		_minBid = (int) Math.round(params.getInt("minBid") * FeatureConfig.CLAN_HALL_MIN_BID_MULTIPLIER);
		_lease = (int) Math.round(params.getInt("lease") * FeatureConfig.CLAN_HALL_LEASE_MULTIPLIER);
			...
			PreparedStatement insertStatement = con.prepareStatement(INSERT_CLANHALL))
		{
			...
			try (ResultSet rset = loadStatement.executeQuery())
			{
				if (rset.next())
				{
					...
					_auctionEndTime = rset.getLong("auctionEndTime");
				...
				else
				{
					...
					insertStatement.setLong(4, 0); // New clanhall should not have active auction timer
	...
	public void updateDB()
	{
			...
			PreparedStatement statement = con.prepareStatement(UPDATE_CLANHALL))
		{
			...
			statement.setLong(3, _auctionEndTime);
	...
	{
		_auctionEndTime = Math.max(0, auctionEndTime);
		updateDB();
	}
