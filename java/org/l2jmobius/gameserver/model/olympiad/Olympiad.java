...
public class Olympiad extends ListenersContainer
{
	...
	private static final Map<Integer, Integer> DAILY_MATCHES = new ConcurrentHashMap<>();
	private static final Map<Integer, Integer> DAILY_CLAN_MATCHES = new ConcurrentHashMap<>();
	private static int _dailyMatchDay = -1;
	
	/** Tracks opponent pairs that have already fought this period (encoded as long). */
	private static final Set<Long> OPPONENT_PAIRS = ConcurrentHashMap.newKeySet();
	...
	private static final String OLYMPIAD_LOAD_NOBLES = "SELECT olympiad_nobles.charId, olympiad_nobles.class_id, characters.char_name, olympiad_nobles.olympiad_points, olympiad_nobles.competitions_done, olympiad_nobles.competitions_won, olympiad_nobles.competitions_lost, olympiad_nobles.competitions_drawn, olympiad_nobles.competitions_done_week, olympiad_nobles.points_last_change FROM olympiad_nobles, characters WHERE characters.charId = olympiad_nobles.charId";
	private static final String OLYMPIAD_SAVE_NOBLES = "INSERT INTO olympiad_nobles (`charId`,`class_id`,`olympiad_points`,`competitions_done`,`competitions_won`,`competitions_lost`,`competitions_drawn`, `competitions_done_week`, `points_last_change`) VALUES (?,?,?,?,?,?,?,?,?)";
	private static final String OLYMPIAD_UPDATE_NOBLES = "UPDATE olympiad_nobles SET olympiad_points = ?, competitions_done = ?, competitions_won = ?, competitions_lost = ?, competitions_drawn = ?, competitions_done_week = ?, points_last_change = ? WHERE charId = ?";
	private static final String OLYMPIAD_GET_HEROS = "SELECT olympiad_nobles.charId, characters.char_name FROM olympiad_nobles, characters WHERE characters.charId = olympiad_nobles.charId AND olympiad_nobles.class_id in (?, ?) AND olympiad_nobles.competitions_done >= " + OlympiadConfig.OLYMPIAD_MIN_MATCHES + " AND olympiad_nobles.competitions_won > 0 ORDER BY olympiad_nobles.olympiad_points DESC, olympiad_nobles.points_last_change ASC, olympiad_nobles.competitions_done DESC, olympiad_nobles.competitions_won DESC";
	private static final String OLYMPIAD_GET_LEGEND = "SELECT olympiad_nobles.charId FROM olympiad_nobles WHERE olympiad_nobles.competitions_done >=" + OlympiadConfig.OLYMPIAD_MIN_MATCHES + " ORDER BY olympiad_nobles.olympiad_points DESC, olympiad_nobles.points_last_change ASC LIMIT 1";
	private static final String GET_ALL_CLASSIFIED_NOBLESS = "SELECT charId from olympiad_nobles_eom WHERE competitions_done >= " + OlympiadConfig.OLYMPIAD_MIN_MATCHES + " ORDER BY olympiad_points DESC, points_last_change ASC, competitions_done DESC, competitions_won DESC";
	private static final String GET_EACH_CLASS_LEADER = "SELECT characters.char_name from olympiad_nobles_eom, characters WHERE characters.charId = olympiad_nobles_eom.charId AND olympiad_nobles_eom.class_id = ? AND olympiad_nobles_eom.competitions_done >= " + OlympiadConfig.OLYMPIAD_MIN_MATCHES + " ORDER BY olympiad_nobles_eom.olympiad_points DESC, olympiad_nobles_eom.points_last_change ASC, olympiad_nobles_eom.competitions_done DESC, olympiad_nobles_eom.competitions_won DESC LIMIT 10";
	private static final String GET_EACH_CLASS_LEADER_CURRENT = "SELECT characters.char_name from olympiad_nobles, characters WHERE characters.charId = olympiad_nobles.charId AND olympiad_nobles.class_id = ? AND olympiad_nobles.competitions_done >= " + OlympiadConfig.OLYMPIAD_MIN_MATCHES + " ORDER BY olympiad_nobles.olympiad_points DESC, olympiad_nobles.points_last_change ASC, olympiad_nobles.competitions_done DESC, olympiad_nobles.competitions_won DESC LIMIT 10";
	...
	private static final String GET_EACH_CLASS_LEADER_CURRENT_SOULHOUND = "SELECT characters.char_name from olympiad_nobles, characters WHERE characters.charId = olympiad_nobles.charId AND (olympiad_nobles.class_id = ? OR olympiad_nobles.class_id = 133) AND olympiad_nobles.competitions_done >= " + OlympiadConfig.OLYMPIAD_MIN_MATCHES + " ORDER BY olympiad_nobles.olympiad_points DESC, olympiad_nobles.points_last_change ASC, olympiad_nobles.competitions_done DESC, olympiad_nobles.competitions_won DESC LIMIT 10";
	...
	private static final String OLYMPIAD_MONTH_CREATE = "INSERT INTO olympiad_nobles_eom SELECT charId, class_id, olympiad_points, competitions_done, competitions_won, competitions_lost, competitions_drawn, points_last_change FROM olympiad_nobles";
	...
	public static final String POINTS_LAST_CHANGE = "points_last_change";
	...
	private void load()
	{
			...
			ResultSet rset = statement.executeQuery())
		{
			...
			while (rset.next())
			{
				...
				statData.set(POINTS_LAST_CHANGE, rset.getLong(POINTS_LAST_CHANGE));
	...
	private void updateCompStatus()
	{
		...
		{
			if (isOlympiadEnd() || !OlympiadConfig.OLYMPIAD_ENABLED)
			{
				return;
			}
	...
	private void scheduleWeeklyChange()
	{
		_scheduledWeeklyTask = ThreadPool.scheduleAtFixedRate(() ->
		{
			...
			clearOpponentPairs();
			LOGGER.info("Olympiad System: Cleared opponent pairs");
	...
	protected synchronized void saveNobleData()
	{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			for (Entry<Integer, StatSet> entry : nobleList)
			{
				...
				final long pointsLastChange = nobleInfo.getLong(POINTS_LAST_CHANGE, 0L);
				...
				try (PreparedStatement statement = con.prepareStatement(toSave ? OLYMPIAD_SAVE_NOBLES : OLYMPIAD_UPDATE_NOBLES))
				{
					if (toSave)
					{
						...
						statement.setLong(9, pointsLastChange);
					...
					else
					{
						...
						statement.setLong(7, pointsLastChange);
						statement.setInt(8, charId);
					}
	...
	protected List<StatSet> sortHerosToBe()
	{
			...
			PreparedStatement statement = con.prepareStatement(OLYMPIAD_GET_HEROS))
		{
			...
			for (int element : HERO_IDS)
			{
				...
				final PlayerClass parent = ClassListData.getInstance().getClass(element).getParentClass();
	...
	private int getOlympiadTradePoint(int objectId)
	{
		...
		// Hero point bonus
		int points = Hero.getInstance().isHero(objectId) || Hero.getInstance().isUnclaimedHero(objectId) ? OlympiadConfig.OLYMPIAD_HERO_POINTS : 0;
	...
	public int getNoblePoints(Player player)
	{
		if (!NOBLES.containsKey(player.getObjectId()))
		{
			...
			statDat.set(POINTS_LAST_CHANGE, 0L);
	...
	{
		return Math.max(OlympiadConfig.OLYMPIAD_MAX_WEEKLY_MATCHES - getCompetitionDoneWeek(objId), 0);
	}
	
	private void checkDailyReset()
	{
		final int today = Calendar.getInstance().get(Calendar.DAY_OF_YEAR);
		if (_dailyMatchDay != today)
		{
			DAILY_MATCHES.clear();
			DAILY_CLAN_MATCHES.clear();
			_dailyMatchDay = today;
		}
	}
	...
	{
		checkDailyReset();
		return DAILY_MATCHES.getOrDefault(objId, 0);
	}
	
	public void addCompetitionDoneDay(int objId)
	{
		checkDailyReset();
		DAILY_MATCHES.merge(objId, 1, Integer::sum);
	}
	
	public int getRemainingDailyMatches(int objId)
	{
		return Math.max(OlympiadConfig.OLYMPIAD_MAX_DAILY_MATCHES - getCompetitionDoneDay(objId), 0);
	}
	
	public int getClanCompetitionDoneDay(int objId)
	{
		checkDailyReset();
		return DAILY_CLAN_MATCHES.getOrDefault(objId, 0);
	}
	
	public void addClanCompetitionDoneDay(int objId)
	{
		checkDailyReset();
		DAILY_CLAN_MATCHES.merge(objId, 1, Integer::sum);
	}
	
	public int getRemainingClanDailyMatches(int objId)
	{
		return Math.max(OlympiadConfig.OLYMPIAD_MAX_DAILY_CLAN_MATCHES - getClanCompetitionDoneDay(objId), 0);
	}
	
	/**
	 * Encode two player object IDs into a unique long key for opponent-pair tracking.
	 */
	private static long pairKey(int id1, int id2)
	{
		final int lo = Math.min(id1, id2);
		final int hi = Math.max(id1, id2);
		return ((long) lo << 32) | (hi & 0xFFFFFFFFL);
	}
	
	/**
	 * Check if two players have already fought this olympiad period.
	 */
	public boolean hasAlreadyFought(int objId1, int objId2)
	{
		return OPPONENT_PAIRS.contains(pairKey(objId1, objId2));
	}
	
	/**
	 * Record that two players have fought this olympiad period.
	 */
	public void addOpponentPair(int objId1, int objId2)
	{
		OPPONENT_PAIRS.add(pairKey(objId1, objId2));
	}
	
	/**
	 * Clear opponent pairs (called on period change).
	 */
	public void clearOpponentPairs()
	{
		OPPONENT_PAIRS.clear();
	}
