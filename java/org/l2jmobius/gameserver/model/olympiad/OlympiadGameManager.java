...
public class OlympiadGameManager implements Runnable
{
	...
	public void run()
	{
		...
		if (Olympiad.getInstance().inCompPeriod())
		{
			...
			boolean readyClan = OlympiadManager.getInstance().hasEnoughRegisteredClan();
			if ((readyClassed != null) || readyNonClassed || readyClan)
			{
				...
				for (int i = 0; i < _tasks.size(); i++)
				{
					...
					synchronized (task)
					{
						if (!task.isRunning())
						{
							// Try clan 9v9 games first.
							if (readyClan)
							{
								newGame = createClanGame(i);
								if (newGame != null)
								{
									task.attachGame(newGame);
									readyClan = OlympiadManager.getInstance().hasEnoughRegisteredClan();
									continue;
								}
								readyClan = false;
							}
	...
	/**
	 * Pick two registered clans and create an OlympiadGameClan match.
	 */
	private AbstractOlympiadGame createClanGame(int stadiumId)
	{
		final java.util.Map<Integer, java.util.List<Integer>> regs = OlympiadManager.getInstance().getRegisteredClanBased();
		if (regs.size() < 2)
		{
			return null;
		}
		
		final java.util.Iterator<java.util.Map.Entry<Integer, java.util.List<Integer>>> it = regs.entrySet().iterator();
		final java.util.Map.Entry<Integer, java.util.List<Integer>> entry1 = it.next();
		final java.util.Map.Entry<Integer, java.util.List<Integer>> entry2 = it.next();
		
		final OlympiadGameClan game = OlympiadGameClan.createGame(stadiumId, entry1.getValue(), entry2.getValue());
		if (game != null)
		{
			regs.remove(entry1.getKey());
			regs.remove(entry2.getKey());
		}
		return game;
	}
