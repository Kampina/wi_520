...
public class BotReportTable
{
	...
	private void handleReport(Player bot, ReportedCharData rcd)
	{
		...
		if ((GeneralConfig.BOTREPORT_ANSWER_TIME > 0) && !bot.hasRequest(CaptchaRequest.class))
		{
			final Captcha captcha = CaptchaGenerator.getInstance().next();
			final CaptchaRequest request = new CaptchaRequest(bot, captcha, GeneralConfig.BOTREPORT_ANSWER_TIME * 1000L);
