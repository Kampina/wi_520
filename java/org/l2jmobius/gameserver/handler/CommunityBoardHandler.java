...
public class CommunityBoardHandler implements IHandler<IParseBoardHandler, String>
{
	...
	public void handleParseCommand(String command, Player player)
	{
		...
		{
			player.sendMessage("You cannot use the Community Board right now.");
			return;
		}
	...
	private boolean isUseBlocked(Player player)
	{
		if (CommunityBoardConfig.COMMUNITYBOARD_PREMIUM_ONLY_OUTSIDE_PEACE && !player.isInsideZone(ZoneId.PEACE) && !player.hasPremiumStatus())
		{
			return true;
		}

		if (!CommunityBoardConfig.COMMUNITYBOARD_ALLOW_WHILE_MOVEMENT_DISABLED && player.isMovementDisabled())
		{
			return true;
		}

		if (!CommunityBoardConfig.COMMUNITYBOARD_ALLOW_WHILE_FLYING && (player.isFlying() || player.isFlyingMounted()))
		{
			return true;
		}

		if (!CommunityBoardConfig.COMMUNITYBOARD_ALLOW_IN_INSTANCE && player.isInInstance())
		{
			return true;
		}

		if (!CommunityBoardConfig.COMMUNITYBOARD_ALLOW_IN_JAIL && player.isJailed())
		{
			return true;
		}

		return !CommunityBoardConfig.COMMUNITYBOARD_ALLOW_OUT_OF_CONTROL && (player.isControlBlocked() || player.hasBlockActions());
	}
