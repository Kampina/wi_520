...
public class DocumentItem extends DocumentBase implements IXmlReader
{
	...
	private void parseItem(Node node) throws InvocationTargetException
	{
		...
		for (n = first; n != null; n = n.getNextSibling())
		{
			...
			{
				makeItem();
				final Map<Stat, Integer> statCounts = new EnumMap<>(Stat.class);
				for (Node b = n.getFirstChild(); b != null; b = b.getNextSibling())
				{
					if ("stat".equalsIgnoreCase(b.getNodeName()))
					{
						final String statName = b.getAttributes().getNamedItem("type").getNodeValue();
						final Stat type = resolveItemStat(statName);
						final double value = normalizeItemStatValue(statName, Double.parseDouble(b.getTextContent()));
						final int statCount = statCounts.merge(type, 1, Integer::sum);
						_currentItem.item.addFunctionTemplate(new FuncTemplate(null, null, getItemStatFunctionName(statCount, value), 0x00, type, value));
					}
				}
			}
	...
	private Stat resolveItemStat(String statName)
	{
		switch (statName)
		{
			case "baseCrit":
			{
				return Stat.CRITICAL_RATE;
			}
			default:
			{
				return Stat.valueOfXml(statName);
			}
		}
	}

	private double normalizeItemStatValue(String statName, double value)
	{
		switch (statName)
		{
			case "baseCrit":
	...
	private String getItemStatFunctionName(int statCount, double value)
	{
		if ((statCount > 1) && isLegacyItemStatMultiplier(value))
		{
			return "mul";
		}
		return "add";
	...
	private boolean isLegacyItemStatMultiplier(double value)
	{
		return (value > 0) && (value != Math.rint(value));
	}
