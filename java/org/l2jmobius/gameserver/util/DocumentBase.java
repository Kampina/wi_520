...
import org.l2jmobius.gameserver.model.conditions.ConditionPlayerIsNoble;
...
import org.l2jmobius.gameserver.model.conditions.ConditionPlayerMinClassLevel;
	...
	protected Condition parsePlayerCondition(Node n, Object template)
	{
		...
		for (int i = 0; i < attrs.getLength(); i++)
		{
			...
			switch (a.getNodeName().toLowerCase())
			{
				...
				case "min_class_level":
				{
					final int minClassLevel = Integer.decode(getValue(a.getNodeValue(), template));
					cond = joinAnd(cond, new ConditionPlayerMinClassLevel(minClassLevel));
					break;
				}
				case "levelrange":
				{
				...
				{
					final boolean val = Boolean.parseBoolean(a.getNodeValue());
					cond = joinAnd(cond, new ConditionPlayerIsNoble(val));
					break;
				}
				case "ispvpflagged":
				{
