...
public class CastleManorManager implements IXmlReader
{
	...
	public CastleManorManager()
	{
		if (false)
		{
			...
			final int maintenanceMin = 0 + 6;
			if (((hour >= 20) && (min >= maintenanceMin)) || (hour < 4) || ((hour == 4) && (min <= 30)))
			{
				_mode = ManorMode.MODIFIABLE;
			}
			else if ((hour == 20) && ((min >= 0) && (min < maintenanceMin)))
			{
			...
			if (!false)
			{
				ThreadPool.scheduleAtFixedRate(this::storeMe, 2 * 60 * 60 * 1000, 2 * 60 * 60 * 1000);
			}
		}
		else
		{
	...
	private void scheduleModeChange()
	{
		...
		switch (_mode)
		{
			case MODIFIABLE:
			{
				_nextModeChange.set(Calendar.HOUR_OF_DAY, 4);
				_nextModeChange.set(Calendar.MINUTE, 30);
			...
			case MAINTENANCE:
			{
				_nextModeChange.set(Calendar.HOUR_OF_DAY, 20);
				_nextModeChange.set(Calendar.MINUTE, 0 + 6);
				break;
			}
			case APPROVED:
			{
				_nextModeChange.set(Calendar.HOUR_OF_DAY, 20);
				_nextModeChange.set(Calendar.MINUTE, 0);
				break;
			}
		}
